...
function updatePositions() {
  ...
  var up = document.body.scrollTop / 1250;

  var phases = [];
  for(var i = 0; i < 5; i++;) {
    phases[i] = Math.sin(up + (i % 5));
  }

  for (var i = 0, max = movePizza.length; i < max; i++) {

    movePizza[i].style.transform = "translateX(" +String(movePizza[i].basicLeft + 100 * phases[i % 5]) + 'px');
  }
